...
@section('title', 'Edit ' . $family->family_name)
@section('subtitle', 'Update family information and details')
    ...
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="flex items-center justify-between mb-8">
            <div class="flex items-center space-x-4">
                <a href="{{ route('families.show', $family) }}" class="inline-flex items-center px-4 py-2 bg-white/80 backdrop-blur-sm border border-gray-200 rounded-xl hover:bg-gray-50 transition-all duration-200">
                    <i class="fas fa-arrow-left mr-2 text-gray-600"></i>
                    Back to Family
                </a>
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">Edit {{ $family->family_name }}</h1>
                    <p class="text-sm text-gray-600">Update family information and details</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-white/80 backdrop-blur-sm rounded-2xl shadow-lg border border-gray-100/50 p-8">
            <form method="POST" action="{{ route('families.update', $family) }}" class="space-y-8">
                @csrf
                @method('PUT')
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" id="family_name" name="family_name" value="{{ old('family_name', $family->family_name) }}" required
                        ...
                        <div class="md:col-span-2">
                            ...
                            <select id="head_id" name="head_id"
                                ...
                                @foreach($members as $member)
                                    <option value="{{ $member->id }}" {{ old('head_id', $family->head_id) == $member->id ? 'selected' : '' }}>
                            ...
                            @error('head_id')
                                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                            <p class="mt-2 text-sm text-gray-500">Select a family member to be the head of this family.</p>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="tel" id="phone" name="phone" value="{{ old('phone', $family->phone) }}"
                        ...
                        <div>
                            ...
                            <input type="email" id="email" name="email" value="{{ old('email', $family->email) }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="city" name="city" value="{{ old('city', $family->city) }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="state" name="state" value="{{ old('state', $family->state) }}"
                        ...
                        <div>
                            ...
                            <input type="text" id="zip_code" name="zip_code" value="{{ old('zip_code', $family->zip_code) }}"
                ...
                <div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', $family->is_active) ? 'checked' : '' }}
                ...
                <div class="flex items-center justify-between pt-6 border-t border-gray-200">
                    <div class="flex items-center space-x-4">
                        <a href="{{ route('families.show', $family) }}" class="px-6 py-3 border border-gray-300 text-gray-700 font-medium rounded-xl hover:bg-gray-50 transition-colors">
                            Cancel
                        </a>
                        <button type="submit" class="px-8 py-3 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-xl hover:from-blue-700 hover:to-indigo-700 transition-all duration-200 shadow-lg hover:shadow-xl transform hover:scale-105">
                    ...
                    <!-- Delete Button -->
                    <button type="button" onclick="confirmDelete()" class="px-6 py-3 bg-red-600 text-white font-medium rounded-xl hover:bg-red-700 transition-colors">
            ...
            <!-- Hidden Delete Form -->
            <form id="delete-form" method="POST" action="{{ route('families.destroy', $family) }}" class="hidden">
                @csrf
                @method('DELETE')
            </form>
        </div>
